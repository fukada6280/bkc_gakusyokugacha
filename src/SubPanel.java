...
public class SubPanel extends JPanel {
    ...
    public SubPanel(MainFrame m, String s, MealData result) {
        ...
        Image small_searchImg = original_searchIcon.getImage()
                .getScaledInstance((int) (original_searchIcon.getIconWidth() * 25 / 256), -1, Image.SCALE_SMOOTH);
        ...
        // 結果: タイトル画像+ラベルの表示
        titleLabel = new JLabel("ガチャ結果", small_searchIcon, JLabel.LEFT);
        titleLabel.setOpaque(true);
        titleLabel.setFont(new Font("Arial", Font.PLAIN, 20));
        titleLabel.setBackground(backGroundColor);
        titleLabel.setPreferredSize(new Dimension(300, 40)); // 大きさを変更 width,height
        titleLabel.setHorizontalTextPosition(JLabel.RIGHT);

        // 結果を表示する
        resultNameLabel = new JLabel();
        resultNameLabel.setFont(new Font("Arial", Font.PLAIN, 20));
        resultNameLabel.setLayout(null); // レイアウトマネージャーを無効化
        resultNameLabel.setBounds(0, 0, 200, 200); // x, y, width, height
        // 結果テキストを改行可能になるよう加工する
        String resultText = "<html>" + result.getName();
        resultText = resultText.replaceAll("＋", "<br>＋ ");
        resultNameLabel.setText(resultText);
        ...
        itemizeLabel.setFont(new Font("Arial", Font.PLAIN, 16));
        ...
        itemizeLabel.setText("<html>・合計価格： " + result.getValue() + "円<br>" +
                                   "・合計カロリー：" + Math.round(result.getKcal()) + "kcal<br>" +
                "・合計タンパク質： " + Math.round(result.getProtein()) + "g<br>" +
                "・合計脂質： " + Math.round(result.getLipid()) + "g<br>" +
                "・合計炭水化物： " + Math.round(result.getCarbohydrate()) + "g<br>" +
                "・合計食塩量： " + Math.round(result.getSalt()) + "g<br>" +
        ...
        toMainBtn.setPreferredSize(new Dimension(300, 40)); // 大きさを変更 width,height
